...
resource "azurerm_storage_account" "PAN_FW_STG_AC" {
  name = join("", [var.StorageAccountName, substr(md5(azurerm_resource_group.PAN_FW_RG.id), 0, 4)])
...
resource "azurerm_subnet_network_security_group_association" "PAN_FW_Subnet0" {
...
resource "azurerm_subnet_network_security_group_association" "PAN_FW_Subnet1" {
...
resource "azurerm_subnet_route_table_association" "PAN_FW_Subnet2" {
...
resource "azurerm_network_interface" "VNIC0" {
  ...
  location            = var.location
  resource_group_name = azurerm_resource_group.PAN_FW_RG.name
  depends_on          = [azurerm_virtual_network.PAN_FW_VNET, azurerm_public_ip.PublicIP_0]
