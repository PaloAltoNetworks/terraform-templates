...
resource "null_resource" "configure_firewall" {
  depends_on = ["azurerm_template_deployment.WeblinkedTemplate"]
  triggers {
    key = "${azurerm_template_deployment.WeblinkedTemplate.id}"
  }

  provisioner "local-exec" {
    command = "./configure_firewall.sh ../../../ansible-playbooks/one_click_multicloud/one_click_azure.yml paloalto Pal0Alt0@123 ${azurerm_public_ip.PublicIP_0.ip_address}"
  }
}
